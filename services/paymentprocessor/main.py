    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as lookup_client:
            ...
            if isinstance(tenant_data, list):
                tenant = next((t for t in tenant_data if str(t.get("reference", "")).strip() == padded_reference), None)
                if not tenant:
            ...
            # Check if tenant already has payments
            has_payments = tenant.get("hasPayments", False)
            logger.info(f"Tenant {tenant_id} hasPayments: {has_payments}")
            ...
            # Determine the endpoint and HTTP method based on hasPayments
            if has_payments:
                payment_url = f"{API_BASE_URL}/api/v2/rents/payment/{tenant_id}/{formatted_term}"
                method = "PATCH"
            else:
                payment_url = f"{API_BASE_URL}/api/v2/rents/payment/{tenant_id}"
                method = "POST"

            logger.info(f"Sending {method} request to URL: {payment_url}")
            ...
            async with httpx.AsyncClient(timeout=30.0) as payment_client:
                if method == "PATCH":
                    ...
                    payment_response = await payment_client.post(payment_url, headers=headers, json=payment_data)
                ...
                return PaymentResult(success=True, tenant_id=tenant_id, message=f"Successfully processed payment for tenant {tenant_id}")
